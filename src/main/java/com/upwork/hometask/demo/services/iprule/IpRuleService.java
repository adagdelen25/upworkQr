...
public class IpRuleService {
  ...
  public SearchOutput search(SearchInput input) {
    ...
    if (StringUtils.isNotBlank(input.getName())) {
      ipRules = ipRuleRepository.findAllBySpecificNameNotContains(
          input.getName(), pageable);
    } else {
      ipRules = ipRuleRepository.findAll(pageable);
    }
    List<SearchOutput.SearchOutputData> searchOutputs =
        ipRules.stream()
            .map(
                ipRule -> {
    ...
    return new SearchOutput(
        searchOutputs, ipRules.getNumber(), ipRules.getTotalPages(), ipRules.getTotalElements());
  }
  ...
  public void update(HttpServletRequest request, Long id, UpdateInput input) {
    IpRule ipRule =
        ipRuleRepository
            .findById(id)
  ...
  public void delete(HttpServletRequest request, Long id) {
    ...
    log.info("An ip rule deleted SS: {} - SE:{}- DS:{} - DE:{} - SubS: {} - SubD : {} A/D : {} ",
        ipRule.getSourceStart(),
        ipRule.getSourceEnd(),
        ipRule.getDestinationStart(),
        ipRule.getDestinationEnd(),
